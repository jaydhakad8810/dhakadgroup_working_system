...
const titles = {
  '/': 'Dashboard', '/trips': 'My Trips', '/trip/new': 'New Trip',
...
export function TopBar() {
  ...
  const isRoot = ['/', '/trips', '/vehicle', '/notifications', '/profile'].includes(pathname)
  const title = titles[pathname] || 'DGSystem Driver'
      ...
      <div className="flex items-center justify-between px-4 py-3.5 max-w-md mx-auto">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {pathname === '/' && <p className="text-gray-500 text-xs mt-0.5">Welcome, {user?.name?.split(' ')[0]}</p>}
          </div>
        </div>
