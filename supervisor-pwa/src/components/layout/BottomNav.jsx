...
import { LayoutDashboard, Building2, Users, ClipboardCheck, FileText, User } from 'lucide-react'
      ...
      <div className="flex items-center justify-around px-1 py-1 safe-area-pb">
        {items.map(({ to, icon: Icon, label }) => (
          <NavLink key={to} to={to} end={to === '/'}
            className={({ isActive }) =>
              `flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl transition-all ${isActive ? 'text-primary-500' : 'text-gray-500'}`
            }>
            {({ isActive }) => (
              <>
